...
日志打印等级可通过 `TRAVEL_PLANNER_LOG_LEVEL` 环境变量来控制。
"""
...
if __name__ == "logging":
    ...
    # 导出常用名称，以便在导入 travel_planner.logging 时方便使用
    ...
    _LOG_DIR = Path("/log/travelplanner")
    _DEFAULT_FORMAT = "%(asctime)s [%(levelname)s] %(name)s: %(message)s"
    _DEFAULT_DATEFMT = "%Y-%m-%d %H:%M:%S"
    _ENV_LEVEL_KEY = "TRAVEL_PLANNER_LOG_LEVEL"
    ...
    def setup_logging(level: Optional[str | int] = None) -> _logging.Logger:
        ...
        - File logging：仅当 ``/log/travelplanner/`` 存在时才启用。
        """
        ...
        # Windows 控制台常为 GBK；模型输出中的货币符号等字符不应让日志处理器崩溃。
