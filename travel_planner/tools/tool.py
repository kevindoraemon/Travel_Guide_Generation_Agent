...
from travel_planner.states import Summary
from travel_planner.prompts import SUMMARIZE_PROMPT, REFINE_ITINERARY_PROMPT
...
summarization_model = get_chat_model("scout_summarizer")
...
def summarize_webpage_content(webpage_content: str) -> str:
    ...
    """
    try:
        # 结构化输出（带 fallback，兼容各类 OpenAI 兼容接口）
        summary = safe_structured_output(summarization_model, Summary, [HumanMessage(content=SUMMARIZE_PROMPT.format(
...
def tavily_search(
    ...
    """根据配置好的tavily API去web上搜索旅游信息（景点、交通、住宿、美食、门票、天气、签证等），并返回做完网页内容摘要后的结果
...
def think_tool(reflection: str) -> str:
    """用于对路书规划进展和决策进行策略反思的工具。
    每次搜索后，使用此工具分析结果并系统地规划下一步行动。这会在路书规划工作流程中故意暂停，以便进行质量决策。

    何时使用它:
    - 收到搜索结果后：我找到了哪些关键信息（景点、价格、时间、交通）？
    - 在决定下一步之前：我的路书是否足够全面？
    - 在评估信息空白时：哪些关键信息仍然缺失（闭馆日、预约要求、淡旺季差异）？
    - 在结束规划之前：我现在能提供一份完整的路书吗？

    反思内容应该强调以下这些方面：
    1. 当前情报的分析 — 我收集到了哪些具体信息？
    2. 差距评估 - 还缺少哪些关键信息？
    3. 质量评估 - 我是否有足够的证据来提供一份好路书？
    4. 战略决策 - 我应该继续搜索还是给出路书？

    Args:
        reflection：您对路书规划进展、发现、存在的差距以及下一步行动的详细反思。
...
def refine_itinerary(trip_brief: Annotated[str, InjectedToolArg],
                     findings: Annotated[str, InjectedToolArg],
                     draft_itinerary: Annotated[str, InjectedToolArg]):

    """根据新搜集的旅游情报(findings)完善目前的路书草稿(draft_itinerary)

    该工具会综合当前所有情报整理输出一份更全面、可执行的路书草稿。

    Args:
        trip_brief：用户的出行需求简报。
        findings：针对用户需求搜集的旅游情报。
        draft_itinerary：基于情报和用户需求的路书草稿。
    ...
    # 组装提示词
    itinerary_prompt = REFINE_ITINERARY_PROMPT.format(
    ...
    # 调用大模型来修正
    itinerary_obj = writer_model.invoke([HumanMessage(content=itinerary_prompt)])

    # 如果返回是message则抽取content字段，否则直接返回
    return getattr(itinerary_obj, "content", itinerary_obj)
...
# 路书精修tool
_refine_itinerary_tool = tool(parse_docstring=True)(refine_itinerary)
